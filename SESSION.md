...
- **census.py** : browserless rate-limit map of X's GraphQL read surface. Pulls the queryId map from `main.js`, sweeps every read op with the 422 primitive, tabulates each bucket. One token per op, reads only (`--include-mutations` to opt in).
