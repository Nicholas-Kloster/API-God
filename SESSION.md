...
- **292 GraphQL ops** (161 query, 131 mutation), extracted by `census.py --deep --inventory`: 157 live in `main.js`, the other 135 in lazy chunks reached via webpack's `_.u` chunk map (URL = `name` + `.` + `hash` + `a.js`, note the `a` suffix). Pattern: `queryId:"..",operationName:"..",operationType:".."`. Write surface includes `CreateTweet`, `CreateNoteTweet`, `CreateRetweet`, `FavoriteTweet`, `CreateBookmark`. Full list in `search/x-op-inventory.json`.
...
- **Bucket map (93 read ops, census.py, 2026-05-30)**: 500 is the default (67 ops); 17 ops throttled to 50 (the scraping-attractive ones: SearchTimeline, UserTweets, UserTweetsAndReplies, Followers, and the Community* timelines); 150 for TweetDetail/UserByScreenName/ProfileFilter; outliers PaymentsUsersTypeahead 5000, TVHomeMixer 900, Viewer 100, FinanceSearchTags 200. UserByRestId/UsersByRestIds are 403 WAF-gated (by-id user resolvers, anti-enumeration). X throttles where abuse concentrates and leaves the rest at 500; a blocked 50-op usually has a 500-op that does the same work.
